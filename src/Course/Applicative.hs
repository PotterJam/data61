...
-- can also be lift3 f x y z = f <$> x <*> y <*> z
...
filtering ::
  ...
  -> List a
  -> f (List a)
-- (sequence $ map p xs) takes us to f (List Bool), then we need to get to f (List a) when Bool is True using a zip type function
filtering p xs = compare xs <$> (sequence $ map p xs)
    where
        compare (y:.ys) (True:.zs)  = y :. compare ys zs
        compare (_:.ys) (False:.zs) = compare ys zs
        compare _       _           = Nil
